...
const MedicalBlogManager = () => {
  ...
  const resetForm = () => {
    ...
    setEditBlogId(null);
    document.getElementById("blogImageInput") && (document.getElementById("blogImageInput").value = "");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.featuredImage && !editBlogId) {
    ...
    try {
      if (editBlogId) {
        // Edit blog
        await axios.put(`${BASE_URL}/api/medical/edit/${editBlogId}`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Blog updated successfully!");
      } else {
        // Add new blog
        await axios.post(`${BASE_URL}/api/medical/add-blog`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      ...
      console.error(err);
      toast.error("Failed to save blog. Try again!");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/medical/delete/${id}`);
      ...
      {showForm && (
        <div className="w-full max-w-4xl mx-auto bg-white shadow-xl rounded-xl p-6 border-t-4 border-[#00796b] mb-6">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            ...
            {/* Short Description */}
            <div className="md:col-span-2">
              <label className="block font-semibold text-gray-700 mb-1">Short Description / Summary *</label>
              <textarea
            ...
            <div key={blog._id} className="bg-white shadow-md rounded-xl overflow-hidden border border-gray-200">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-500 my-1">
                  {blog.mainCategory} {blog.subCategory && `> ${blog.subCategory}`}
                </p>
                ...
                <div className="flex justify-end mt-3 gap-2">
                  <button
                    ...
                    className="px-3 py-1 bg-yellow-400 rounded-lg text-white hover:opacity-90"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => handleDelete(blog._id)}
                    className="px-3 py-1 bg-red-500 rounded-lg text-white hover:opacity-90"
                  >
                    Delete
                  </button>
                </div>
